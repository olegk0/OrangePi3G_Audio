...
inline status_t AudioLock::lock_timeout(const uint32_t milliseconds) {
    ...
    return mutex.try_lock_for(std::chrono::milliseconds(milliseconds)) ? 0 : -1;
}
...
inline status_t AudioCondition::wait(AudioLock &mutex) {
    std::unique_lock<std::timed_mutex> lk(mutex.mutex, std::defer_lock);
...
*/
    if (mCond.wait_for(lk, std::chrono::nanoseconds(reltime)) == std::cv_status::timeout)
        return -1;
    return 0;
}
