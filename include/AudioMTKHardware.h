...
class AudioMTKHardware /*: public AudioHardwareBase*/ {
   public:
    AudioMTKHardware();
    ...
    virtual int xWayPlay_PutDataToSpeaker(char *target_ptr, uint16_t num_data_request);
   ...
   protected:
    ...
    virtual bool isModeInCall(int mode) {
        // return ((mode == AUDIO_MODE_IN_CALL) || (mode == AUDIO_MODE_IN_CALL_2) || (mode == AUDIO_MODE_IN_CALL_EXTERNAL) || (mode == AUDIO_MODE_IN_COMMUNICATION));
    }
